...
# Genetic Algorithm using Real-Valued Representation
...
def bin(number):
    return "{0:8b}".format(number).replace(' ','0')


def initialize(population, max_limit, min_limit):
    population_list = []
    for x in range(0, population):
        number = round(random.uniform(max_limit,min_limit), 2)
        pre_binary = int(256 * ((number - min_limit) / (max_limit - min_limit)))
        population_list.append(bin(pre_binary))
    return population_list


def to_value(representation):
    return int(-1 + ((representation/256)*(2 + 1)))
...
def evaluate(population):
    ...
    for chaval in population:
        value = to_value(int(chaval, 2))
        y = value*math.sin(10*math.pi*value) + 2
